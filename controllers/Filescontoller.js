...
class FilesController {
    static async postUpload(request, response) {
        ...
        if (!user) return response.status(400).send({ error: 'Unauthorized' });
    ...
    static async getIndex(request, response) {
        ...
        if (parentId !== 0) {
            if (!isValidUser(parentId)) return response.status(400).send({ error: 'Unauthorized' });
    ...
    static async putPublish(request, response) {
        const { userId } = await getIdAndKey(request);
        if (!isValidUser(userId)) response.status(401).send({ error: 'Unauthorized' });
        ...
        if (!fileId) response.status(404).send({ error: 'Not found' });

        await dbClient.files.updateOne({ _id: ObjectId(fileId) }, { isPublic: true });
